...
%Purpose: Create poster-ready nondimensional plot for rmax
...
ylabel('$\\log_2(\widetilde{r}_m/\widetilde{r}_m^*)$','Interpreter','Latex','FontSize',36)
...
hold on
if(m==1)
    text1=text(1.8,2.8,sprintf('$\\alpha = $ %5.2f',exp_rmVpf),'fontweight','bold','FontSize',48,'Color',[0 .5 0])
    set(text1,'HorizontalAlignment','center','VerticalAlignment','middle','Interpreter','Latex');
else
    text1=text(1.8,3.9,sprintf('$\\alpha = $ %5.2f',exp_rmVpf),'fontweight','bold','FontSize',48,'Color',[0.5 0 0])
    set(text1,'HorizontalAlignment','center','VerticalAlignment','middle','Interpreter','Latex');    
end
