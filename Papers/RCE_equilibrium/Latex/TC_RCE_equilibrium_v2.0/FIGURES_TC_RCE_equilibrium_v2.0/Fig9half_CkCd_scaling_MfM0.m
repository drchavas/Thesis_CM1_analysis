...
%%Mf = Vm*rm
%%M0 = .5*f*r0^2
    Mf = Vmax_equil_g.*rmax_equil_g*1000;
    M0 = .5*fcor_all.*((1000*r0Lil_equil_g).^2);
    data_pl = Mf./M0;
%    L = data_pl - MfM0_adj.*Vmax_equil_g_ts_min./mpi_all;
%    U = MfM0_adj.*Vmax_equil_g_ts_max./mpi_all - data_pl;
...
ER11_eq38 = ((CkCd_smooth)./2).^(1./(2-(CkCd_smooth)));

ER11_eq38_ctrl = ((CkCd(i_ctrl))./2).^(1./(2-(CkCd(i_ctrl))));
MfM0_adj = ER11_eq38_ctrl/data_pl(i_ctrl);


clr1 = [0 0 1];

%% Plot error bars
h1 = plot(x,data_pl,'LineStyle','none','Color',[.5 .5 .5])
hold on

h2 = plot(x_smooth,ER11_eq38,'k--',x,MfM0_adj*data_pl,'x','MarkerEdgeColor',clr1,'MarkerSize',20,'LineWidth',2)
xlabel('$\\log_2(C_k/C_d)$','Interpreter','Latex','FontSize',18)
ylabel(sprintf('$M_f/M_0$'),'Interpreter','Latex','FontSize',18)
...
print -dpdf -r300 Fig9half_CkCd_scaling_MfM0.pdf