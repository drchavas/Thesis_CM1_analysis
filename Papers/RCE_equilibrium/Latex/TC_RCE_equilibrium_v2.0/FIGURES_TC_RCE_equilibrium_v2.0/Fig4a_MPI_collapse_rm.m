...
%Purpose: Paper-ready MPI collapse plot for rmax, with error bars
...
%{
    ...
    VmVp = .7790;   %=rmax_equil_g_CTRL/mpi_CTRL
    mpi_all = rmax_equil_g/VmVp; %%u_sfc adjustment DRC 07 Jun 2012
    end
%}
...
%    end
    ...
    %rmax_equil_g_ctrl = rmax_equil_g_dynamic_all(i_ctrl);
...
%    subplot(3,1,1)
    ...
    %data_temp = rmax_equil_g_dynamic_all(i_sort);
    rmax_equil_g_ts_min_temp = rmax_equil_g_ts_min(i_sort);
    rmax_equil_g_ts_max_temp = rmax_equil_g_ts_max(i_sort);
    
    data_pl = log2(data_temp./rmax_equil_g_ctrl);
    dat_max = max(dat_max,max(data_pl));
    dat_min = min(dat_min,min(data_pl));

    L = data_pl - log2(rmax_equil_g_ts_min_temp./rmax_equil_g_ctrl);
    U = log2(rmax_equil_g_ts_max_temp./rmax_equil_g_ctrl) - data_pl;
...
%legend([h1],sim_sets_str,'Orientation','horizontal','Location','SouthOutside','EdgeColor','black','FontSize',18)
%set(h,'Position',get(h,'Position')+[0 -.25 0 0])
grid on
