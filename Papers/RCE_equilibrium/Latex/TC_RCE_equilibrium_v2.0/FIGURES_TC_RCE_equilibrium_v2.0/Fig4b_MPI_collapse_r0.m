%Fig4b_MPI_collapse_r0.m

%Created: 24 Jul 2012, Dan Chavas

%Purpose: Paper-ready MPI collapse plot for r0_Lil, with error bars
...
%{
    ...
    VmVp = .7790;   %=r0Lil_equil_g_CTRL/mpi_CTRL
    mpi_all = r0Lil_equil_g/VmVp; %%u_sfc adjustment DRC 07 Jun 2012
    end
%}
...
%    subplot(3,1,1)
    ...
    r0Lil_equil_g_ts_min_temp = r0Lil_equil_g_ts_min(i_sort);
    r0Lil_equil_g_ts_max_temp = r0Lil_equil_g_ts_max(i_sort);
    
    data_pl = log2(data_temp./r0Lil_equil_g_ctrl);
    dat_max = max(dat_max,max(data_pl));
    dat_min = min(dat_min,min(data_pl));

    L = data_pl - log2(r0Lil_equil_g_ts_min_temp./r0Lil_equil_g_ctrl);
    U = log2(r0Lil_equil_g_ts_max_temp./r0Lil_equil_g_ctrl) - data_pl;
...
ylabel('$\\log_2(r_0/r_0^*)$','Interpreter','Latex','FontSize',18)
...
print -dpdf -r300 Fig4b_MPI_collapse_r0.pdf
