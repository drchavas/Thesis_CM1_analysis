...
%ydat = r0Lil_equil_g./(mpi_all./fcor_all);
...
%% Determine optimal value at crossing point
...
h2 = semilogx(xvals_pl,ydat,'x','MarkerEdgeColor',clr1,'MarkerSize',10);
hold on
h3 = semilogx(x_lh_Vpf,y_VmVp,'r-.','LineWidth',.5);
semilogx(x_fit,y_fit,'k--')
semilogx(lh_Vpf_optimal,y_VmVp(1),'g.','MarkerSize',20)
y_optim = 0:1.1*max(ydat)/100:1.1*max(ydat);
semilogx(lh_Vpf_optimal*ones(length(y_optim)),y_optim,'-.','Color',[.8 .8 .8],'LineWidth',.5)
xlabel('$l_h/\frac{Vp}{f}$','Interpreter','Latex','FontSize',18)
ylabel('$V_m/V_p$','Interpreter','Latex','FontSize',18)
legend([h2 h3],{'$data$','$\frac{1}{\sqrt{2}}$'},'Interpreter','Latex','FontSize',18,'FontWeight','bold')

axis([.9*min(xvals_pl) 1.1*max(xvals_pl) 0 1.1*max(ydat)])

end
%}
%alpha_Vm
lh_Vpf_optimal
lh_optimal = lh_Vpf_optimal*(91.583/5e-5)
