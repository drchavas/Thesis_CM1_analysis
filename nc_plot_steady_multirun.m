...
%% USER INPUT %%%%%%%%%%%%%%%%%%
subdir_pre='CTRL_icRCE/extra/';    %general subdir that includes multiple runs within
...
rmax_plot = 600;    %[km]; highest value plotted
...
subdirs = {
'CTRLv0qrhSATqdz5000_nx3072_2'
'CTRLv0qrhSATqdz5000_nx3072_dx16'
'CTRLv0qrhSATqdz5000_nx3072_dx64'
...
for rr=1:numruns
    ...
    data_tmean=zeros(xf-x0+1,zf-z0+1);
...
%            data = data - squeeze(vinterp);
        ...
        data=squeeze(data);
        data_tmean=data_tmean(1:nx_sub,1:nz_sub);
        data_tmean=data_tmean+data/(i_tf-i_t0+1);
    ...
    data_tmean = data_tmean(i_xvals',i_zvals');
    data_tmean=double(data_tmean);  %pcolor requires double format

    %%CALCULATE THE DESIRED RADIUS OF v_usr FOR THE TIME-MEAN PROFILE
    if(r_ts==1)
        xres = dx;
        i_max = find(data_tmean==max(max(data_tmean)));  %index of max wind speed
        i_max = i_max(1);
        if(strmatch(v_usr,'MAX'))
            if(~isempty(i_max))
                r_usr_tmean(rr)=xres*i_max-.5*xres;
            else
                r_usr_tmean(rr)=NaN;
            end
        else
            if(strcmp(var,'vinterp') || strcmp(var,'vg'))
                temp1=find(data_tmean<=v_usr);
                temp2=find(temp1>i_max,1);
                i_vusr_out=temp1(temp2);
                clear temp1 temp2
                if(~isempty(i_vusr_out))
                    v_out = data_tmean(i_vusr_out);   %v at first point where v<=v_usr
                    v_in = data_tmean(i_vusr_out-1);  %v at last point where v>=v_usr
                    r_usr_tmean(rr) = xres*(i_vusr_out-(v_out-v_usr)/(v_out-v_in)) - .5*xres;   %r=dr/2 @ i=1
                    if(r_usr_tmean(rr)<0 || r_usr_tmean(rr)>100000000)
                        r_usr_tmean(rr)=NaN;
                    end
                else
                    v_out = NaN;
    ...
    %%CALCULATE Vmax, Rmax FOR THE TIME-MEAN PROFILE
    i_max = find(data_tmean==max(data_tmean));  %index of max wind speed
    Vmaxs(rr) = data_tmean(i_max);
    ...
    if(make_plot==1)
        ...
        %%BASIC STATISTICS
        data_min = min(min(data_tmean));
        data_max = max(max(data_tmean));
        ...
        if(length(zvals)>1)   %rz color plot
            ...
            if(numvars > 1)
                pcolor(xmat,zmat,data_tmean')
                shading flat
                shading interp
            else
                contourf(xmat,zmat,data_tmean',20)
            end
            ...
            plot(xmat,data_tmean',pl_clrs{rr},'LineWidth',2)
...
if(make_plot==1)
    ...
    if(strcmp(v_usr,'MAX'))
        if(strcmp(var,'vg'))
            input_ylabel=sprintf('radius of %s gradient wind [km]',v_usr);
            input_title1=sprintf('Time series: Radius of %s gradient wind [km] (%s)',v_usr,smooth_str);
        else
        ...
        if(strcmp(var,'vg'))
            input_ylabel=sprintf('radius of%5.1f m/s gradient wind [km]',v_usr);
            input_title1=sprintf('Time series: Radius of%5.1f m/s gradient wind [km] (%s)',v_usr,smooth_str);
        else
